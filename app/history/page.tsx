        ...
        <section className="mb-10"><h2 className="text-2xl font-bold mb-4">🎮 Ceremonies</h2>
          <div className="space-y-3">
            ...
            {ceremonies.map((c: any) => (
              <a key={c.id} href={c.type === "review" ? `/review/${c.id}` : `/retro/${c.id}`} className="glass rounded-xl p-4 flex justify-between items-center hover:border-gold transition block">
                <span className="font-bold">{c.type === "review" ? "📊 Sprint Review" : ((MODE_CONFIG as any)[c.gameMode]?.name ?? c.gameMode)}</span>
